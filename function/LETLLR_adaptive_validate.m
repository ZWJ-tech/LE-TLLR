...
function [ BestParameter, BestResult ] = LETLLR_adaptive_validate( train_data, train_target, oldOptmParameter)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% tune the best parameters for LETLLR by crossvalidation
% Input
    ...
    for i=1:length(alpha_searchrange) % alpha
        for j=1:length(beta_searchrange) % beta
            for k = 1:length(gamma_searchrange) % gamma
                    fprintf('\n-   %d-th/%d: search parameter alpha and beta for LETLLR, alpha = %f, beta = %f, and gamma = %f',index, total, alpha_searchrange(i), beta_searchrange(j), gamma_searchrange(k));
                    index = index + 1;
